...
def detect_columns(blocks, k_max=3):
    if not blocks:
        return np.array([])
    xs = np.array([[ (b[0] + b[2]) / 2 ] for b in blocks])
...
def sort_blocks(blocks):
    ...
    for b, col in zip(blocks, labels):
        ...
        enriched.append((col, round(b[1], 1), b[0], b))
...
def score_block(content, norm, toc_map):
    """Weighted classification from ocr-n1, ensuring block_type parity."""
    scores = {
        "math": 0,
        "heading": 0,
        "list": 0,
...
def extract_blocks(pdf_path, max_pages=None):
    ...
    for i in pages_to_process:
        ...
        # 2. Reading Order & Engine Routing
        route_to_llm = False
        ...
        # Apply Column-Aware Sorting
        sorted_text_blocks = sort_blocks(raw_text_blocks)
        ...
        if route_to_llm:
            ...
            if mdfile:
                ...
                for b in raw_chunks:
                    ...
                    btype = "formula" if has_math else score_block(b, norm, toc_map)
        ...
        # Apply Column-Aware Sorting to Fast Branch
        page_blocks = sort_blocks(page_blocks)
